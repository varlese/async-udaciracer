	...
	const newRace = createRace( player_id, track_id )
		...
		.then ( () => {
			...
			.then( ( race_id ) => {
				startRace( race_id - 1 )
				.catch( ( error ) => {
					const html = renderError( 'There was an error when starting the race.', error )
					renderAt( 'body', html )
				} )
			} )
			...
			.then( runRace )
			.catch( ( error ) => {
				const html = renderError( 'There was an error starting the countdown.', error )
				renderAt( 'body', html )
			} )
		} )
		.catch( ( error ) => {
			const html = renderError( 'There was an error when creating the race.', error )
			renderAt( 'body', html )
		} )
}
	...
	return new Promise( resolve => {
		...
		const intervalID = setInterval( () => {
			...
			const race = getRace( race_id - 1 )
				...
				.catch( ( error ) => {
					const html = renderError( 'There was an error fetching the race.', error )
					renderAt( 'body', html )
				} )
		}, 500 )
	} )
...
const renderError = ( message, error ) => {
	...
	<main>
		...
		<p>${message}</p>
		${error.message}
		<a href="/race" class="button">Try again</a>
		</section>
	</main>
	`
}
